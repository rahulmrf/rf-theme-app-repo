/* All links that are not internal, opens in a new tab */
...
/* Start helper check in viewport */
...
/* End helper check in viewport */
/* START Sticky cta button */
var stickyButtonSelector = '.on-scroll-not-visible-sticky',
    placeholderButtonClass = 'placeholder-sticky-button',
    applyStickyButtonClass = 'rf-sticky-button-cta';
    
if ($(stickyButtonSelector).length > 0) {

  /* Pre pending div so we don't lose reference on viewport visible/not visible */
  $(stickyButtonSelector).after('<div class="' + placeholderButtonClass + '"></div>');
  
  $( window ).on( "scroll", function() {
    var elementNotOnViewport = !$('.' + placeholderButtonClass).isOnScreen();
    //var elementNotOnViewport = !$(stickyButtonSelector).parent().isOnScreen();
    
      
    $(stickyButtonSelector).toggleClass(applyStickyButtonClass, elementNotOnViewport);
  } );
}
...
/* this implementation just print the final number wich is calculated with hours passed and a start number (baseNumber) */
$('.counter-second').each(function() {
  var $this = $(this),
      now = new Date(),
      startDateCounting = new Date("2022-04-04"),
      start = 74906,
      baseNumber = 74906,
      countTo = baseNumber + (now - startDateCounting)/(60*60*1000);
...
async function loadCountryCode() {
    let url = '/browsing_context_suggestions.json',
        ...
        euCountryCodes = ["AT", "BE", "BG", "HR", "CY", 
                          ...
                          "PL", "PT", "RO", "SK", "SI", 
                          "ES", "SE", "NO"];
    if (undefined != obj.detected_values.country.handle) {
...
$(document).ready(function(){
  loadCountryCode();
 console.log('loading country cod');
});
/* CODE to automatically add a free prod with automatic dicsount. will work on cart page. */
/*
$(document).ready(function(){
  if ($('body.cart').length > 0) {
    var cartContents = fetch(window.Shopify.routes.root + 'cart.js')
      ...
      .then(cart => { 
        var cart,
          idFreeItemToAdd = [38828201106],
          idItemsValid = [38828187154,38828187218,13185286635543,31657205202967,13185286668311,31657208021015,13185286701079,31657208053783,13185286733847,31657208086551],
          cartIds = [];

        cart.items.forEach(function(item){
          cartIds.push(item.id);
        });
        const foundValid = idItemsValid.some(r=> cartIds.includes(r)),
          notFoundFree = !idFreeItemToAdd.some(r=> cartIds.includes(r));

        if (foundValid && notFoundFree) {
          if( window.localStorage ) {
            if( !localStorage.getItem('firstLoadCartPage') ) {
              $.post(window.Shopify.routes.root + 'cart/update.js',"updates[" + idFreeItemToAdd[0] + "]=1");
              localStorage['firstLoadCartPage'] = true;
              setTimeout(() => {
                document.location.href = '/cart';
              }, "500")
            } else {
              localStorage.removeItem('firstLoadCartPage');
            }
          }
        }
      });
...
$(document).ready(function () {
  ...
  $('.slideshow-wrapper').infiniteslide({
    'speed': 50,
    'direction':'right',
    'pauseonhover':true,
    'responsive':true
  ...
  setTimeout(function() {$('.slideshow-wrapper').fadeTo( 'slow', 1 );}, 500);

  $('.less-velocity').on('click',function(){
    var currentDuration = $('.slideshow-wrapper').css('animation-duration').replace("s", ""),
        durationUpdate = parseInt(currentDuration) + 20;
    if (durationUpdate < 210 ) {
      $(this).css({'opacity':1});
      $('.more-velocity').css({'opacity':1});
      $('.slideshow-wrapper').fadeOut('fast',function(){
        $(this).css({'animation-duration': durationUpdate + 's'}).fadeIn('fast');
      });
    } else {
  ...
  $('.more-velocity').on('click',function(){
    var currentDuration = $('.slideshow-wrapper').css('animation-duration').replace("s", ""),
        durationUpdate = parseInt(currentDuration) - 20;
    
    if (durationUpdate > 10 ) {
      $(this).css({'opacity':1});
      $('.less-velocity').css({'opacity':1});
      $('.slideshow-wrapper').fadeOut('fast',function(){
        $(this).css({'animation-duration': durationUpdate + 's'}).fadeIn('fast');
      });
    } else {
  ...
  // check position every x sec and play and pause base on position 
  setInterval(function() {
    $( ".slideshow-wrapper .slide" ).each(function(index) {
        let $this = $( this ),
            rect = $this.get( 0 ).getBoundingClientRect(),
            wrapper = $( ".slideshow-wrapper" ).get( 0 ).getBoundingClientRect(),
            upperLimit = 1100,
            downerLimit = 650;
        const $currentVideo =$this.find('video');

        if (1200 > $( window ).width()) {
          upperLimit = $( window ).width()*2;
          downerLimit = 200;
        }
      
        if(downerLimit < rect.right && rect.right < upperLimit) {
...
$(document).ready(function () {
  ...
  $('.pause-play').on('click',function(){
    if ($(this).hasClass('paused')) {
      ...
      $('.slideshow-wrapper').css({'animation-play-state': 'running'});
    } else {
      $(this).addClass('paused');
      $('.slideshow-wrapper').css({'animation-play-state': 'paused'});
    }
  });

  
  var i = 0, timeOut = 0, sliderDurationSet = 80;
  
  $('.more-velocity-x3').on('mousedown touchstart', function(e) {
    var currentDuration = sliderDurationSet,
        durationUpdate = Math.floor(parseInt(currentDuration)/3);
    ...
    var currentDuration = sliderDurationSet;
      ...
      $('.slideshow-wrapper').fadeOut('fast',function(){
        $(this).css({'animation-duration': currentDuration + 's'}).fadeIn('fast');
      });
  ...
  $(".connect-me").each(function() {
    ...
    $(".connect-me").each(function() {
      if (id2 > 1) {
        var prevId = id2 - 1,
            line = new LeaderLine(
            document.getElementById('id${'+prevId+'}'),
            document.getElementById('id${'+id2+'}'),
            { dash: { animation: true , len: 5, gap: 6},
               startSocketGravity: [0, 90],
               endSocketGravity: [0, -90],
               startSocket: 'bottom', 
               endSocket: 'top',
               color : 'white',
               size : 2,
